...
| 任务 | 描述 | 状态 | 提交 |
|-----|------|:----:|------|
| Task 11 | UniDataModule 基类 | ✅ 完成 | 054c81f |
...
src/Core/Data/UniConnectionManager.pas    - 连接管理器
...
#### 服务层 (9 个文件)
```
src/Core/Services/UniAuthService.pas      - 认证服务
src/Core/Services/UniAuthService.Intf.pas - 认证接口
src/Core/Services/UniPermissionManager.pas - 权限管理
src/Core/Services/UniPermissionManager.Intf.pas - 权限接口
src/Core/Services/UniMenuManager.pas      - 菜单管理
src/Core/Services/UniMenuManager.Intf.pas - 菜单接口
...
#### UI 组件 (8 个文件)
```
src/UI/Components/UniPropertyEditor.pas   - 属性编辑器
...
src/UI/Components/UniPropertyEditor.dfm
```
...
服务通过 `TUniServices` 服务定位器统一管理：

```pascal
TUniServices = class
public
  class procedure Initialize(Connection: TFDConnection);
  class property ConnectionManager: IUniConnectionManager;
  class property MetadataManager: IUniMetadataManager;
  class property AuthService: IUniAuthService;
  class property PermissionManager: IUniPermissionManager;
  class property MenuManager: IUniMenuManager;
...
| **L** - 里氏替换 | 接口定义明确，实现可互换 | IUniAuthService 接口 |
