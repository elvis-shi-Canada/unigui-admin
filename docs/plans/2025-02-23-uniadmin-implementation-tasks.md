...
- Create: `src/Core/Session/UniAdminSession.pas`
...
type
  ...
  // TUniAdminSession 类
  TUniAdminSession = class(TObject)
  private
    FInfo: TSessionInfo;
  ...
  public
    constructor Create(const Session: TUniAdminSession);
    function GetUserID: Integer;
    function GetUserName: string;
    function GetRealName: string;
    function HasPermission(const PermissionCode: string): Boolean;
  ...
  public
    constructor Create(const Session: TUniAdminSession; const DBConfig: IDatabaseConfig);
    function GetUserContext: IUserContext;
    function GetCurrentUserID: Integer;
    function GetCurrentUserName: string;
    function GetCurrentTime: TDateTime;
...
constructor TUniAdminSession.Create(const SessionID: string);
begin
  inherited Create;
...
destructor TUniAdminSession.Destroy;
begin
...
function TUniAdminSession.CreateUserContext: IUserContext;
begin
  Result := TUserContextImpl.Create(Self);
end;

function TUniAdminSession.CreateExecutionContext: IExecutionContext;
begin
  // 数据库配置将在后面设置
  Result := TExecutionContextImpl.Create(Self, nil);
end;

procedure TUniAdminSession.Login(const AUserID: Integer; const AUserName, ARealName: string);
begin
  FLock.Enter;
  try
...
procedure TUniAdminSession.Logout;
begin
  FLock.Enter;
  try
    DeactivateAllPlugins;
...
function TUniAdminSession.IsLoggedIn: Boolean;
begin
  FLock.Enter;
  try
...
procedure TUniAdminSession.RegisterPlugin(const PluginName: string; const Plugin: IPlugin);
begin
  FLock.Enter;
  try
...
function TUniAdminSession.GetPlugin(const PluginName: string): IPlugin;
begin
  FLock.Enter;
  try
...
procedure TUniAdminSession.ActivateAllPlugins;
begin
  FLock.Enter;
  try
...
procedure TUniAdminSession.DeactivateAllPlugins;
begin
  FLock.Enter;
  try
    for var LPlugin in FPlugins.Values do
...
procedure TUniAdminSession.SetValue(const Key: string; const Value: Variant);
begin
  FLock.Enter;
  try
...
function TUniAdminSession.GetValue(const Key: string): Variant;
begin
  FLock.Enter;
  try
...
function TUniAdminSession.TryGetValue(const Key: string; out Value: Variant): Boolean;
begin
  FLock.Enter;
  try
...
constructor TUserContextImpl.Create(const Session: TUniAdminSession);
begin
  inherited Create;
  FSession := Session;
end;
...
constructor TExecutionContextImpl.Create(const Session: TUniAdminSession; const DBConfig: IDatabaseConfig);
begin
  inherited Create;
  FSession := Session;
...
```bash
git add src/Core/Session/UniAdminSession.pas
git commit -m "feat: 创建会话管理类 TUniAdminSession"
```
...
**Files:**
- Create: `src/Core/Plugin/UniAdminModuleRegistry.Intf.pas`
- Create: `src/Core/Plugin/UniAdminModuleRegistry.pas`
...
type
  ...
  private
    class var FInstance: IUniAdminModuleRegistry;
    FPluginClasses: TDictionary<string, TPluginClassInfo>;
    FDependencies: TList<TDependencyInfo>;

    class function GetInstance: IUniAdminModuleRegistry; static;
  ...
  public
    ...
    // 全局访问点
    class property Instance: IUniAdminModuleRegistry read GetInstance;
  end;
...
procedure TUniAdminModuleRegistry.RegisterPluginClass(const PluginName: string; const PluginClass: TClass;
  const PluginType: string; const Description: string; const Version: string);
var
...
procedure TUniAdminModuleRegistry.RegisterDependency(const ModuleName, DependsOn: string);
var
...
function TUniAdminModuleRegistry.CreatePlugin(const PluginName: string; const UserContext: IUserContext;
  const ExecutionContext: IExecutionContext): IPlugin;
var
  LInfo: TPluginClassInfo;
...
function TUniAdminModuleRegistry.IsRegistered(const PluginName: string): Boolean;
begin
  Result := FPluginClasses.ContainsKey(PluginName);
end;

function TUniAdminModuleRegistry.GetPluginInfo(const PluginName: string): TPluginClassInfo;
begin
  if not FPluginClasses.TryGetValue(PluginName, Result) then
    raise Exception.CreateFmt('Plugin %s not found', [PluginName]);
end;

function TUniAdminModuleRegistry.GetAllPlugins: TArray<TPluginClassInfo>;
begin
  FPluginClasses.Values.ToArray(Result);
end;

function TUniAdminModuleRegistry.GetDependencies(const PluginName: string): TArray<string>;
var
  LList: TList<string>;
begin
...
function TUniAdminModuleRegistry.GetDependents(const PluginName: string): TArray<string>;
var
  LList: TList<string>;
begin
  LList := TList<string>.Create;
...
function TUniAdminModuleRegistry.ValidateDependencies: Boolean;
var
...
function TUniAdminModuleRegistry.HasCircularDependency(const StartNode, CurrentNode: string;
  Visited: TDictionary<string, Boolean>): Boolean;
var
  LDeps: TArray<string>;
begin
...
procedure TUniAdminModuleRegistry.EnsureDependencyGraph;
begin
  // 验证所有依赖都指向已注册的插件
  for var LDep in FDependencies do
  begin
...
class procedure PluginClassRegistry.Register(const PluginName: string; const PluginClass: TClass;
  const PluginType: string; const Description: string; const Version: string);
begin
  TUniAdminModuleRegistry.Instance.RegisterPluginClass(PluginName, PluginClass,
    PluginType, Description, Version);
end;

initialization
  // 确保注册表实例被创建
  TUniAdminModuleRegistry.GetInstance;

finalization
  // 清理全局实例
  TUniAdminModuleRegistry.FInstance := nil;
...
```bash
git add src/Core/Plugin/UniAdminModuleRegistry.Intf.pas src/Core/Plugin/UniAdminModuleRegistry.pas
...
**Files:**
- Create: `src/Core/Config/UniAdminConfigService.Intf.pas`
- Create: `src/Core/Config/UniAdminConfigService.pas`
...
type
  ...
  public
    constructor Create(const ModuleName: string; const Config: TJSONObject;
      const ConfigService: IUniAdminConfigService);
    destructor Destroy; override;
  ...
  public
    ...
    // 全局访问
    class function GetInstance: IUniAdminConfigService; static;
    class property Instance: IUniAdminConfigService read GetInstance;
  end;
...
begin
  if Assigned(FConfigService) then
    TUniAdminConfigService(FConfigService as TObject).DoNotifyChange(FModuleName + '.' + Key);
end;
...
begin
  if FInstance = nil then
    FInstance := TUniAdminConfigService.Create;
  Result := FInstance;
end;
...
function TUniAdminConfigService.GetAppConfig(const Key: string): string;
var
...
function TUniAdminConfigService.GetAppConfigInteger(const Key: string; Default: Integer): Integer;
var
  LStr: string;
begin
...
function TUniAdminConfigService.GetAppConfigBoolean(const Key: string; Default: Boolean): Boolean;
var
  LStr: string;
begin
...
procedure TUniAdminConfigService.SetAppConfig(const Key, Value: string);
var
  LParts: TArray<string>;
...
procedure TUniAdminConfigService.LoadFromFile(const FileName: string);
var
  LJSON: string;
  LRoot: TJSONValue;
begin
...
procedure TUniAdminConfigService.SaveToFile(const FileName: string);
begin
  FFileName := FileName;
  TFile.WriteAllText(FileName, FAppConfig.ToJSON, TEncoding.UTF8);
end;

procedure TUniAdminConfigService.Reload;
begin
  if FFileName <> '' then
    LoadFromFile(FFileName);
end;

procedure TUniAdminConfigService.AddChangeNotification(const Callback: TProc<string>);
begin
  FChangeCallbacks.Add(Callback);
end;

procedure TUniAdminConfigService.RemoveChangeNotification(const Callback: TProc<string>);
begin
  FChangeCallbacks.Remove(Callback);
end;

procedure TUniAdminConfigService.DoNotifyChange(const Key: string);
begin
...
initialization
  TUniAdminConfigService.GetInstance;
...
```bash
git add src/Core/Config/UniAdminConfigService.Intf.pas src/Core/Config/UniAdminConfigService.pas
...
- Create: `src/Core/Plugin/UniAdminPluginLoader.pas`
...
```bash
git add src/Core/Plugin/UniPlugin.Types.pas src/Core/Plugin/UniAdminPluginLoader.pas
...
uses
  UniAdminConfigService, UniAdminModuleRegistry;
...
begin
  ...
  TUniAdminConfigService.Instance.LoadFromFile(LConfigFile);

  // 初始化插件注册表
  TUniAdminModuleRegistry.Instance;
end;
...
begin
  ...
  // 清理配置服务
  TUniAdminConfigService.Instance := nil;
end;
...
uses
  ...
  UniContext, UniPlugin.Intf, UniAdminSession;
...
type
  ...
  public
    { Public declarations }
    function GetOrCreateSession(const SessionID: string): TUniAdminSession;
...
begin
  FSessions := TDictionary<string, TUniAdminSession>.Create;
end;

procedure TUniGUIMainModule.UniGUIMainModuleDestroy(Sender: TObject);
var
  LPair: TPair<string, TUniAdminSession>;
begin
...
function TUniGUIMainModule.GetOrCreateSession(const SessionID: string): TUniAdminSession;
begin
  if not FSessions.TryGetValue(SessionID, Result) then
  begin
    Result := TUniAdminSession.Create(SessionID);
...
uses
  ...
  UniAdminModuleRegistry.Intf, UniPlugin.Intf, UniAdminPluginLoader;
...
uses
  UniAdminModuleRegistry, UniAdminPluginLoader;

constructor TPluginManagerFrame.Create(AOwner: TComponent);
begin
  inherited;
  FRegistry := TUniAdminModuleRegistry.Instance;
  FLoader := TUniAdminPluginLoader.Create(FRegistry);
...
uses
  UniAdminConfigService;

constructor TConfigManagerFrame.Create(AOwner: TComponent);
begin
  inherited;
  FConfigService := TUniAdminConfigService.Instance;
  LoadConfigList;
end;
...
uses
  ...
  UniPlugin, UniContext, UniAdminModuleRegistry.Intf;
...
### Task 11: 创建 UniAdminDataModule 基类

**Files:**
- Create: `src/Core/Data/UniAdminDataModule.pas`
- Create: `src/Core/Data/UniAdminDataModule.dfm`
...
```bash
git add src/Core/Data/UniAdminDataModule.pas src/Core/Data/UniAdminDataModule.dfm
...
**Files:**
- Create: `src/Core/Data/UniAdminConnectionManager.pas`
- Create: `src/Core/Data/UniAdminConnectionManager.Intf.pas`
...
uses
  System.SysUtils, System.Classes, System.Generics.Collections,
  FireDAC.Comp.Client,
  UniAdminConnectionManager.Intf, UniAdminConfigService.Intf;
...
```bash
git add src/Core/Data/UniAdminConnectionManager.pas src/Core/Data/UniAdminConnectionManager.Intf.pas
...
**Files:**
- Create: `src/Core/Metadata/UniAdminMetadataCache.pas`
- Create: `src/Core/Metadata/UniAdminMetadataCache.Intf.pas`
...
```bash
git add src/Core/Metadata/UniAdminMetadataCache.pas src/Core/Metadata/UniAdminMetadataCache.Intf.pas
...
**Files:**
- Create: `src/Core/Auth/UniAdminAuthService.pas`
- Create: `src/Core/Auth/UniAdminAuthService.Intf.pas`
...
```bash
git add src/Core/Auth/UniAdminAuthService.pas src/Core/Auth/UniAdminAuthService.Intf.pas
...
**Files:**
- Create: `src/Core/Menu/UniAdminMenuManager.pas`
- Create: `src/Core/Menu/UniAdminMenuManager.Intf.pas`
...
```bash
git add src/Core/Menu/UniAdminMenuManager.pas src/Core/Menu/UniAdminMenuManager.Intf.pas
...
**Files:**
- Create: `src/Core/Permission/UniAdminPermissionManager.pas`
- Create: `src/Core/Permission/UniAdminPermissionManager.Intf.pas`
...
```bash
git add src/Core/Permission/UniAdminPermissionManager.pas src/Core/Permission/UniAdminPermissionManager.Intf.pas
...
| 任务 | 描述 | 文件 |
|-----|------|------|
| Task 18 | CRUD 基础组件 | TUniAdminModel.pas |
| Task 19 | CRUD 基类窗体 | TBaseCrudFrame.pas |
| Task 20 | 属性编辑器 | UniAdminPropertyEditor.pas |
| Task 21 | 主窗体框架 | MainFrame.pas |
| Task 22 | 登录窗体 | LoginForm.pas |
| Task 23 | 布局导航 | UniAdminLayout.pas |
| Task 24 | 服务集成 | UniAdminServices.pas |
| Task 25 | 主题管理 | UniAdminTheme.pas |
